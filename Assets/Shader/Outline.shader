Shader "Hidden/Outline"
{
    ...
    float _Threshold;
    float _OutlineThickness; // New property for thickness
    ...
    float4 FullScreenPass(Varyings varyings) : SV_Target
    {
        ...
        float outlineThickness = _OutlineThickness * 0.01; // Convert percentage to value
        ...
        if (Luminance(outline.rgb) < luminanceThreshold)
        {
            ...
            {
                float2 uvN = uv + _ScreenSize.zw * _RTHandleScale.xy * samplingPositions[i] * outlineThickness; // Scale sampling position by thickness
